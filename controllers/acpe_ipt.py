    ...
    return dict(x=XML(f'''
        ...
        <form id="form5" width="100%">
            <table width='100%' class='table1'><tr>
                ...
                <td><div> <input name="t1" id="t1" onkeyup="ajax('{URL('spks','acpe', 'fingilish2',args='fin_to_fa')}', ['t1'], 't3',cp('t2'));" onchange="cp('t2')" /></div></td>
        ...
        <script>
            ...
            '''))    
def fingilish2():
    #sample use : spks/acpe_ipt/fingilish2/fin_to_fa/abc
    import k_finglish as fe
    args=request.args
    ...
    if args and len(args)>1:
        ...
        if args[0]=='fin_to_fa':
            return fe.fin_to_fa(args[1])#request.vars['t1'])
        elif args[0]=='fa_to_fin':
            return fe.fa_to_fin(args[1])#request.vars['t4'])          
def one():
    return dict()
