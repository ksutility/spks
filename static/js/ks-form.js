...
function app_key(t) {
	/*
	021123
		- بهبود عملکرد دکمه بازگشت - عدم نیاز به پر کردن فیلدهای خالی
		-- improve "r:return" task 
	...
	*/
	...
	if (t=="R") {
		...
		if (confirm(msg) == true) {
			var msg =	"با توجه به اهمیت موضوع برای انجام این کار لازم است کد زیر را وارد نمایید "
		...
		if (!(is_validateForm())){
			alert("فیلدهای مشخص شده با رنگ نارنجی خالی می باشند")
			return false;
		}	
		else { 
			//if ( isvalid_re()==true)
			if ( true==true) { re = confirm("ARE You Sure2 ? \n \n" + "آیا مطمئن هستید ؟" );	} 	
		}
	}
	if (re == true) {
...
//------------------------------------------------------------------------------------------------------------
function is_validateForm() {
	/*
		021123
		 تهیه برنامه جاوا برای بررسی پر شدن فیلدهای خالی برای مدیریت بهتر دکمه های نهایی یک مرحله 
	*/
	...
	for (let i = 0; i < collection.length; i++) {
		...
		*/
		...
		if (elem.hasAttribute('required')  )  {
			...
			//tt1+='--'+vv
			//alert(collection[i].type+"\n"+tt1+"\n"+att_list(collection[i]));
		}
		
	}
	function att_list(obj){
		const nodeMap = obj.attributes;
		let text = "";
...
function isvalid_re(){
			...
			if (alert_msg !='') { alert( alert_msg );re=false;}
			return re;
		}
function isvalid(){
		return true;
