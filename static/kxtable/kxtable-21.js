...
var xsort_c_name;
var xsort_c_mode;
var xlink="/prj/static/kxtable/" //000825
xsort_c_mode=true;
...
*/	
	function kxtable_make_table(in_cookiePage,htm_obj_name){
	...
	var ttt='\
			...
			<div style="width:100%">\
				...
				' + txt_add1 + '\
				<textarea name="kx_show_txt" id="kx_show_txt" rows="1" style="resize:vertical" class="kxtable" style="width:100%"></textarea> \
			<div>\
		...
		'
		if ( htm_obj_name === undefined) {
			document.write(ttt);
		} else {
			document.getElementById(arguments[1]).innerHTML = ttt ;
		}
		$(function() {
		...
		txt ="<tr id='kxtable_head_tr'>";
			...
			for (i=0;i <  kxtable_data_cols.length;i++) {
				...
				txt1 = "    <th ><div id='div-head-" + n + "'  class='context-menu-one' " + w + ">\
					" +  t1 + "<img  class='kxt_sort_sng' src='" + xlink + fn + "'></img> </div></th>";
				txt += txt1;
			}
			txt += "</tr>" ;
	...
	function kxtable_set_table_head_jquery(){	
			...
			$(".context-menu-one" ).droppable({
				...
				drop: function( event, ui ) {
					...
					if (re!=0) {
						if(event.shiftKey){
							swich_ar_items(kxtable_Arrangement,i_drop,i_darg)
						} else {	
							if (i_darg<i_drop){
								for(k=i_darg;k<i_drop;k++){
									swich_ar_items(kxtable_Arrangement,k,k+1)	
								 }
							} else {
								for(k=i_darg;k>i_drop;k--){
									swich_ar_items(kxtable_Arrangement,k,k-1)	
...
function kxtable_show_sort_sign(col){
		...
		$(".kxt_sort_sng").attr("src", xlink+"bg.png")
		$("#kxtable_head_tr th:nth-child(" + col + ") div img").attr("src", xlink+ re + ".gif")
	}	
//-------------------------------------------------------------------------------------------------------------------	
...
function getCookie(cname) {
    ...
    for(var i=0; i<ca.length; i++) {
        var c = ca[i];
        while (c.charAt(0)==' ') c = c.substring(1);  //remove left spaces
        if (c.indexOf(name) == 0) return c.substring(name.length, c.length);  //if (c is start by name) then (return string after name) else (goto next coki )
    }
    return "";
}
//-----------------------------------------------------------------------
function checkCookie() { //test
...
function kxflot_pr(x_c) { 
	//prepare file for mkake graph by flot
	...
	function copy_js_object(mainObj) {
   ...
   window.open(xlink+"kxflot.htm","","width=800,height=450") 
}
//----------------------------------------------------------
function swich_ar_items(ar,a,b){
	var c;
	c=ar[a];
	ar[a]=ar[b];
	ar[b]=c;
